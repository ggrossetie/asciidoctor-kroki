...
To link from an element in a PlantUML diagram to another Antora page, first set `kroki-default-format: svg` and enable inline SVG (either `allow-uri-read: true`, or `kroki-fetch-diagram: true` with `kroki-default-options: inline`; see xref:syntax.adoc#options[Options]).
Inlining the SVG is required for the link to be clickable: a diagram referenced as a plain image target has no interactive content.
...
The `subs=attributes` substitution is required: a diagram block is verbatim by default, so `{site-url}` and the other attributes would otherwise be sent to Kroki unresolved.
